...
public static class SetsAndMaps
{
    ...
    public static bool IsAnagram(string word1, string word2)
    {
        ...
        for (int i = 0; i < word1.Length; i++)
        {
            ...
            if (!letterCounts.ContainsKey(c1))
                letterCounts[c1] = 0;
            letterCounts[c1]++;

            if (!letterCounts.ContainsKey(c2))
                letterCounts[c2] = 0;
            letterCounts[c2]--;
        }
    ...
    public static string[] EarthquakeDailySummary()
    {
        ...
        // Extract earthquakes with valid magnitude and location
        return featureCollection.Features
            .Where(f => f.Properties != null && f.Properties.Mag.HasValue && !string.IsNullOrWhiteSpace(f.Properties.Place))
            .Select(f => $"Magnitude {f.Properties.Mag} earthquake at {f.Properties.Place}")
            .ToArray();
    }
}
